...
### nginx reverse proxy — TLS and plain-HTTP listeners must be separate `server` blocks
The app sits behind nginx (`/etc/nginx/sites-available/dmis.conf`, not part of this repo), which proxies `/dmis/` to Payara's `http-listener-1` on `127.0.0.1:8083` across four public listeners: `8181 ssl`, `443 ssl`, `8080`, `80`. Payara itself only ever sees plain HTTP from nginx and has no idea a TLS-terminating proxy sits in front — so any server-side redirect it issues (e.g. the post-login `faces-redirect=true` navigation) naturally comes back as `http://<host>:<port>/...` using whatever port was in the client's original `Host` header.

**Do not put all four listeners in one `server` block with a blanket `proxy_redirect http:// https://;`.** That rewrite is only correct for the TLS listeners (8181/443); applied to the plain listeners (8080/80) it upgrades the redirect's scheme to `https://` while leaving the port unchanged, sending the browser to attempt a TLS handshake on a port that only speaks plain HTTP — which fails immediately with `ERR_SSL_PROTOCOL_ERROR`. This bit us for real: after merging PRs #248/#250, logging in over port 8080 hit exactly this error.

The fix is two separate `server` blocks with identical `location` blocks: one for `8181 ssl`/`443 ssl` (keeps `proxy_redirect http:// https://;`), one for `8080`/`80` (no rewrite, or `proxy_redirect off;`, so a plain-HTTP redirect stays plain HTTP). Confirmed via `curl` on the login POST: before the split, `Location: https://localhost:8080/...`; after, `Location: http://localhost:8080/...`. If `dmis.conf` is ever consolidated back into one block, this bug will silently return.
